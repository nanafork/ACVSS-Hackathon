...
\begin{itemize}\small
  \item \textbf{A lesion is} an area of abnormal tissue, so for us a tumor: one
        ...
        as a whole tumor.
  \item \textbf{Erasure is} the failure we set out to fix: the reconstruction smooths a real
        tumor away, so the scan looks normal when it is not. It is also called
        pseudo-normalization. \textbf{The erasure rate is} the number of true lesions the
