...
torch.manual_seed(0)"""),
    ...
    full = make_dataset("brats", root=DATA_ROOT, modality="t1c", size=SIZE,
                        ...
                        cache_path="/kaggle/working/brats_slices.npz")
    # Split by CASE: adjacent slices of one scan are near-duplicates, so a
    # slice-level split would leak anatomy into the test set.
    train_idx, test_idx = full.split_by_case(val_frac=0.2, seed=0)
